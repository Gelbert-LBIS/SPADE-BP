classdef SANTEC_550 < handle
    ...
    methods
        ...
        function setSweep(this,Lmin,Lmax,rate,cyc,del)
            ...
            fprintf(this.hw,[':WAV:SWE:CYCL ' num2str(cyc)]);
			fprintf(this.hw,[':WAV:SWE:DEL ' num2str(del)]);
			fprintf(this.hw,':TRIG:OUTP 3'); % step trig
			fprintf(this.hw,':TRIG:INP:STAN 0'); % dont wait for trig
			fprintf(this.hw,':TRIG:OUTP:STEP 0.05'); % 5pm trig steps
        end

        function SweepGO(this)
            fprintf(this.hw,':WAV:SWE 1'); % sweep
        end
		
		function SETMOD(this)
			%setWlF(this,0);
            % fprintf(this.hw,':AM:SOUR 3'); % freq mod
            % fprintf(this.hw,':AM:STAT 1'); % enable modulation
			% allways on
        end
