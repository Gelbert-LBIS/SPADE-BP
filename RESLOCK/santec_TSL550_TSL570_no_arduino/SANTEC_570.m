classdef SANTEC_570 < handle
    ...
    methods
        ...
        function setSweep(this,Lmin,Lmax,rate,cyc,del)
            % Set Continuous sweep mode.
            writeline(this.hw,':WAV:SWE:MOD 1'); % cont 1 way
            ...
            writeline(this.hw,[':WAV:SWE:CYCL ' num2str(cyc)]);
            writeline(this.hw,[':WAV:SWE:DEL ' num2str(del)]);
            writeline(this.hw,':TRIG:OUTP 3'); % step trig
            writeline(this.hw,':TRIG:INP:STAN 0'); % dont wait for trig
            writeline(this.hw,':TRIG:OUTP:STEP 0.01'); % 10pm trig steps
            writeline(this.hw,':TRIG:OUTP:SETT 0'); % const wl
        end

        function SweepGO(this)
            writeline(this.hw,':WAV:SWE 1'); % sweep
        end

         function SETMOD(this)
            writeline(this.hw,':AM:SOUR 3'); % freq mod
            writeline(this.hw,':AM:STAT 1'); % enable modulation
        end
